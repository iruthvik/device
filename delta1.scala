...
val windowedStream = pulsarStream
  .withColumn("window_start", 
    expr(s"timestamp - INTERVAL CAST(FLOOR(UNIX_TIMESTAMP(timestamp) / (${windowDuration.toLong * 60})) * (${windowDuration.toLong * 60}) AS INT) SECONDS"))
  .withColumn("window_end", 
    expr(s"timestamp + INTERVAL (${windowDuration.toLong * 60} - 1) SECONDS"))
  .groupBy(
