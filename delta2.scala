...
// Define the window duration in seconds (5 minutes)
val windowDurationInSeconds = 5 * 60
...
val windowedStream = pulsarStream
  .withColumn("window_start", 
    expr(s"timestamp - INTERVAL CAST(FLOOR(UNIX_TIMESTAMP(timestamp) / $windowDurationInSeconds) * $windowDurationInSeconds AS INT) SECONDS"))
  .withColumn("window_end", 
    expr(s"timestamp + INTERVAL ($windowDurationInSeconds - 1) SECONDS"))
  .groupBy(
    window(col("window_start"), s"${windowDurationInSeconds} seconds").alias("window"),
    col("Devicename"),
    col("inteface")
  )
  .agg(
